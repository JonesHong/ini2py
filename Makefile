...
# 運行測試並生成覆蓋率報告
test-coverage:
	python -m pytest tests/ --cov=ini2py --cov-report=html --cov-report=term
...
lint:
	...
	flake8 ini2py/ tests/
	@echo "運行 mypy..."
	mypy ini2py/ --ignore-missing-imports
	@echo "檢查 import 順序..."
	isort --check-only ini2py/ tests/
...
check-setup:
	...
	@echo "項目版本: $$(python -c 'import ini2py; print(ini2py.__version__)')"
...
# 版本檢查
version:
	@python -c "import ini2py; print(f'當前版本: {ini2py.__version__}')"
	@echo "pyproject.toml 版本: $$(grep '^version' pyproject.toml | cut -d'\"' -f2)"

# 創建新版本標籤
tag-version:
	@VERSION=$$(python -c "import ini2py; print(ini2py.__version__)"); \
